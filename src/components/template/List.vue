<template>
  <div class="list">
    <AppBar v-bind="$attrs" v-on="$listeners" pageTitle="还款列表" guolv @getScreenParams="getScreenParams"></AppBar>
    <SearchInputBar placeholderText="搜索项目编号/客户姓名/手机号码" @searchInputBarChange="searchInputBarChange"></SearchInputBar>
    <div class="contentBox">
      <div class="content">
        <mu-load-more :refreshing="loadUpdate.refreshing" @refresh="refresh" :loading="loadUpdate.loading" @load="load" :loaded-all="loadUpdate.loadedAll">
          <ListItem :listData="listData" @goPage="goPage(goPageName,goPageParams)"></ListItem>
        </mu-load-more>
      </div>
    </div>
  </div>
</template>

<script>
import AppBar from '@components/AppBar'
import SearchInputBar from "@components/SearchInputBar";
import ListItem from "@components/template/ListItem";
export default {
  components: {
    AppBar, SearchInputBar, ListItem
  },
  props: {
    pageSource:{
      type:String,
      default:""
    },
    goPageName:{
      type:String,
      default:""
    }
  },
  data() {
    return {
      listData:[
        {
          num:'Z190215000049877',
          status:'还款中',
          name:'杜斯',
          phone:'13695872546',
          carId:'6211365998523',
          money:'90954.72',
          dateTime:'2020/04/03',
          qishu:'13/36',
          yinhuan:'2526.52',
          updateTime:'2019/9/6'
        }
      ],
    }
  },
  created () {
    this.apiMethods.getListCallback(this,this.pageSource);
  },
  mounted () {
    
  },
  methods: {
    searchInputBarChange(val){
      console.log(val)
    },
    getScreenParams(data){
      console.log(data)
      this.screenData = data;
      this.resetListHandle();
      this.apiMethods.getListCallback(this,this.pageSource,true);
    },
    // 下拉刷新
    refresh() {
      console.log('下拉刷新')
      this.refreshHandle();
      this.apiMethods.getListCallback(this,this.pageSource,true);
    },
    // 上拉加载
    load() {
      console.log('上拉加载')
      this.loadHandle();
      this.apiMethods.getListCallback(this,this.pageSource,true);
    },
  }
}
</script>

<style lang="less" scoped>
  .list{
    .contentBox{
      height: 100vh;
      overflow: hidden;
    }
    .content{
      height: calc(100vh - 104px);
      overflow-y: scroll;
      margin-top: 104px;
    }
  }
</style>
